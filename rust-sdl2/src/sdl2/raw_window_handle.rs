    ...
    fn window_handle(&self) -> Result<WindowHandle<'_>, HandleError> {
        ...
        match wm_info.subsystem {
            ...
            SDL_SYSWM_WINDOWS => {
                ...
                let hinstance = unsafe { wm_info.info.win }.hinstance as *mut std::ffi::c_void;
                let hinstance = std::num::NonZeroIsize::new(hinstance as isize);
            ...
            SDL_SYSWM_WINRT => {
                ...
                let std_window = unsafe { wm_info.info.winrt }.core_window;
                let std_window = std::ptr::NonNull::<core::ffi::c_void>::new(core_window);
                let std_window = match std_window {
                    Some(cw) => cw,
                    None => {
                        return Err(HandleError::Unavailable);
                    }
            ...
            SDL_SYSWM_WAYLAND => {
                ...
                let surf = unsafe { wm_info.info.wl }.surface as *mut std::ffi::c_void;
                let surf = std::ptr::NonNull::<core::ffi::c_void>::new(surf);
            ...
            SDL_SYSWM_COCOA => {
                ...
                let ns_view = std::ptr::NonNull::<core::ffi::c_void>::new(self.context().metal_view);
            ...
            SDL_SYSWM_UIKIT => {
                ...
                let ui_window = unsafe { wm_info.info.uikit }.window as *mut std::ffi::c_void;
                let ui_window = std::ptr::NonNull::<core::ffi::c_void>::new(ui_window);
            ...
            SDL_SYSWM_ANDROID => {
                ...
                let a_native_window = unsafe { wm_info.info.android }.window as *mut std::ffi::c_void;
                let a_native_window = std::ptr::NonNull::<core::ffi::c_void>::new(a_native_window);
    ...
    fn display_handle(&self) -> Result<DisplayHandle, HandleError> {
        ...
        match wm_info.subsystem {
            ...
            SDL_SYSWM_WAYLAND => {
                ...
                let display = unsafe { wm_info.info.wl }.display as *mut std::ffi::c_void;
                let display = std::ptr::NonNull::<core::ffi::c_void>::new(display);
                match display {
                    Some(display) => {
            ...
            SDL_SYSWM_X11 => {
                ...
                let display = unsafe { wm_info.info.x11 }.display as *mut std::ffi::c_void;
                let display = std::ptr::NonNull::<core::ffi::c_void>::new(display);
...
pub mod windows {
    ...
    pub struct WinRtHandles {
        pub std_window: *mut std::ffi::c_void,
    }

    impl Default for WinRtHandles {
        fn default() -> Self {
            WinRtHandles {
                std_window: std::ptr::null_mut(),
            }
        }
    }
}
